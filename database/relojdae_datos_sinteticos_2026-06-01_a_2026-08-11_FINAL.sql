...
-- ------------------------------------------------------------
-- VALIDACION PREVIA: detectar colisiones de codigos o zk_user_id
-- ------------------------------------------------------------
DO $$
BEGIN
    IF EXISTS (
        ...
        WHERE codigo_empleado IN (
            'EMP-9901','EMP-9902','EMP-9903','EMP-9904',
            'EMP-9905','EMP-9906','EMP-9907','EMP-9908'
        )
        AND COALESCE(observaciones,'') <> 'DATOS_SINTETICOS_RELOJ_2026'
    ) THEN
        RAISE EXCEPTION 'Ya existe uno de los codigos EMP-9901 a EMP-9908 en datos reales.';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM personal.empleados
        WHERE zk_user_id IN ('9101','9102','9103','9104','9105','9106','9107','9108')
        AND COALESCE(observaciones,'') <> 'DATOS_SINTETICOS_RELOJ_2026'
    ) THEN
        RAISE EXCEPTION 'Ya existe uno de los zk_user_id 9101 a 9108 en datos reales.';
    END IF;
END $$;
...
WITH personas(codigo_empleado, nombres, apellido_paterno, apellido_materno, correo, zk_user_id) AS (
    VALUES
        ('EMP-9901','Ana','Torres','Molina','ana.torres.test@dae.local','9101'),
        ('EMP-9902','Carlos','Mendoza','Ruiz','carlos.mendoza.test@dae.local','9102'),
        ('EMP-9903','Daniela','Ruiz','Vargas','daniela.ruiz.test@dae.local','9103'),
        ('EMP-9904','Eduardo','Salas','Ortega','eduardo.salas.test@dae.local','9104'),
        ('EMP-9905','Fernanda','Lopez','Castro','fernanda.lopez.test@dae.local','9105'),
        ('EMP-9906','Gabriel','Ortega','Navarro','gabriel.ortega.test@dae.local','9106'),
        ('EMP-9907','Isabel','Castro','Mendez','isabel.castro.test@dae.local','9107'),
        ('EMP-9908','Jorge','Navarro','Luna','jorge.navarro.test@dae.local','9108')
)
...
WITH empleados_test AS (
    ...
    FROM personal.empleados
    WHERE codigo_empleado LIKE 'EMP-990%'
),
dias AS (
...
--   AND codigo_empleado LIKE 'EMP-990%';
-- COMMIT;
